...
generated quantities {
  // =========================================================================
  // 1. THE EXPORT ZONE (Root Level)
  // Only variables declared here will be saved to the CSV and returned to R.
  // =========================================================================

  vector[total_Z] Z_elements;

  // Base linear interpolation paths (N_quad points per drifter)
  array[D] vector[N_quad] p_x_base;
  array[D] vector[N_quad] p_y_base;
  array[D] vector[N_quad] v_x_base;
  array[D] vector[N_quad] v_y_base;

  // The Z-projected basis matrices (much smaller than Phi_flat)
  // You will multiply these by your posterior 'w' draws in R
  matrix[N_quad, total_M] Phi_Z_flat;
  matrix[N_quad, total_M] Phi_d_Z_flat;

  // =========================================================================
  // 2. THE HIDDEN ZONE (Local Scope)
  // Everything inside these braces is destroyed before the CSV is written.
  // =========================================================================
  {
    ...
    // Compute the massive intermediate matrices (Hidden from CSV)
    matrix[N_quad, total_K] Phi_flat;
    matrix[N_quad, total_K] Phi_d_flat;
    ...
    for(d in 1:D){
      ...
      for(k in 1:cur_K){
        ...
        cur_Phi_d[,k] = sin(((cur_quad_grid - drifter_boundaries[d,1])/cur_Lt) * pi() * (k-1)) * -1.0 * pi() * (k-1) / cur_Lt;
      }
    ...
    // --- Precomputations (Assigning to the exported variables) ---
    for (d in 1:D) {
      int k_start = K_starts[d];
    ...
    // Surface Physics Prior computations (Hidden, assuming you don't need them in R)
    ...
    // for (i in 1:N_models) {
    //   vector[M_Surface+1] sqrt_spd = sqrt(sqrt(2*pi()) * fixed_ls[i] * exp(-0.5 * square(fixed_ls[i] * omega_surface)));
